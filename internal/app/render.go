...
func renderNginx(cfg Config) string {
	...
	return fmt.Sprintf(`# Generated by NexaGate. Local changes will be replaced.
server {
    listen 80;
    listen [::]:80;
    server_name %s;
...
`, serverName, cfg.ACMEWebroot, cfg.Ports.PanelHTTPS, cfg.Ports.PanelHTTPS, cfg.Ports.PanelHTTPS, serverName,
		cfg.CertFile, cfg.KeyFile, cfg.Ports.WebSocketTLS, cfg.Ports.WebSocketTLS, serverName,
		cfg.CertFile, cfg.KeyFile, cfg.WebSocketPath, cfg.Ports.WebSocketLocal)
}

func yamlQuote(value string) string { return "'" + strings.ReplaceAll(value, "'", "''") + "'" }

type Links struct{ Hysteria, XHTTPReality, RawReality, WebSocketTLS string }
